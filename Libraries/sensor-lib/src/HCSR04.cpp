...
uint16_t HCSR04::checkEcho(void)
{
    ...
    if(state & ECHO_RECD)
    {
        ...
        float actualDistance = (echoLength-0.553)/57.1;
        Serial.print('\t');
        Serial.print(actualDistance);} */
    return echoLength;
}

uint16_t HCSR04::readMCP3002(bool force)
{
    uint16_t retVal = 0;
...
uint16_t HCSR04::readASCII(void)
{
